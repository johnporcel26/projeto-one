...
export const itemDefinitions: Record<string, ItemDefinition> = {
  ...
  fruit_baku_baku: { id: "fruit_baku_baku", displayName: "Baku Baku no Mi", category: "fruit", icon: "fruit_baku", stackable: false, sellValue: 0, tradeable: true },
};
...
export const skillDefinitions: Record<string, SkillDefinition> = {
  ...
  baku_cannon: { id: "baku_cannon", displayName: "Tiro de Canhão", description: "Disparo à distância que causa 150% do ataque base.", cooldownMs: 15_000, manaCost: 0, targetType: "enemy", status: "AVAILABLE" },
  baku_bite: { id: "baku_bite", displayName: "Mordida", description: "Mordida curta que recupera 10% do dano real causado.", cooldownMs: 10_000, manaCost: 0, targetType: "enemy", status: "AVAILABLE" },
  baku_armor: { id: "baku_armor", displayName: "Blindagem", description: "Bloqueia os próximos 2 ataques válidos.", cooldownMs: 20_000, manaCost: 0, targetType: "self", status: "AVAILABLE" },
...
export const fruitDefinitions: Record<string, FruitDefinition> = {
  ...
  fruit_baku_baku: { id: "fruit_baku_baku", name: "Baku Baku no Mi", rarity: "epic", icon: "fruit_baku", description: "Poder de canhão, mordida e blindagem.", skillIds: ["baku_cannon", "baku_bite", "baku_armor", "baku_placeholder"] },
};
...
export type AutoHuntSkillPolicy = { enabled: boolean; priority: number; condition: "ALWAYS" | "HP_BELOW"; hpThresholdPercent: number; };
export interface AutoHuntSettings { autoUseConsumables: boolean; hpPotionEnabled: boolean; hpThresholdPercent: number; manaPotionEnabled: boolean; manaThresholdPercent: number; potionPreference: "SMALL_FIRST" | "LARGE_FIRST" | "SMART"; utilityMode: "AUTO" | "SLOT_1" | "SLOT_2"; basicAttackEnabled: boolean; targetPriority: AutoHuntTargetPriority; skillPolicies: Record<string, AutoHuntSkillPolicy>; }
export const defaultAutoHuntSettings: AutoHuntSettings = { autoUseConsumables: false, hpPotionEnabled: true, hpThresholdPercent: 40, manaPotionEnabled: false, manaThresholdPercent: 30, potionPreference: "SMART", utilityMode: "AUTO", basicAttackEnabled: true, targetPriority: "NEAREST", skillPolicies: {} };
export interface WalletSnapshot { berries: number; rubies: number; reservedBerries: number; reservedRubies: number; }
export interface PlayerSnapshot { id: string; name: string; level: number; totalXp: number; xpIntoCurrentLevel: number; xpRequiredForNextLevel: number | null; wallet: WalletSnapshot; x: number; y: number; direction: Direction; state: PlayerState; stats: Stats; resources: PlayerResources; activeFruitId: FruitId | null; ownedFruitIds: FruitId[]; inventory: InventoryStack[]; /** Locks protect the whole stack and are enforced by the server. */ lockedItemIds: ItemId[]; /** Optional personal vault. Both inventory and storage are intentionally unlimited. */ storage: InventoryStack[]; utilitySlots: [ItemId | null, ItemId | null]; autoHunt: AutoHuntState; autoHuntSettings: AutoHuntSettings; /** Server timestamps; the HUD derives the visible skill cooldown from these. */ skillCooldownEndsAt: Partial<Record<string, number>>; /** Server-authoritative effect expiry timestamps for player visuals. */ statusEffectEndsAt: Partial<Record<"sube_defense" | "sube_evasion" | "guro_spin" | "baku_armor", number>>; }
...
export interface EnemySnapshot { id: string; type: "enemy_alvida" | "enemy_buffalo" | "enemy_wapol"; x: number; y: number; hp: number; maxHp: number; state: EnemyState; direction: Direction; }
...
export interface GameSnapshot { area: "forest_alvida" | "beach_buffalo" | "ice_mountain" | "pirate_ship"; player: PlayerSnapshot; enemies: EnemySnapshot[]; npcs: NpcSnapshot[]; huntAnalyzer: HuntAnalyzerSnapshot; /** The 20 most recent completed Hunt Analyzer sessions, newest first. */ huntAnalyzerHistory: HuntAnalyzerSnapshot[]; combatFeedback: CombatFeedbackSnapshot[]; auction: AuctionSnapshot; /** Validated server catalog lets the client resolve Admin-created IDs without code changes. */ contentCatalog: { items: ItemDefinition[]; fruits: FruitDefinition[]; public: PublicCatalog; discovery: CatalogDiscoverySnapshot }; }
