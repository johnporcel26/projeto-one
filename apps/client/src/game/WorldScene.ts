...
export class WorldScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    for (let frame = 1; frame <= 5; frame++) this.load.image(`guro_explosion_${frame}`, Asset.guroExplosion(frame));
  ...
  private playGuroExplosion(x: number, y: number): void { const fx = this.add.image(x, y, "guro_explosion_1").setDisplaySize(190, 190).setOrigin(.5).setDepth(2990); let frame = 1; this.time.addEvent({ delay: 100, repeat: 4, callback: () => { frame += 1; if (frame <= 5) fx.setTexture(`guro_explosion_${frame}`); } }); this.tweens.add({ targets: fx, alpha: 0, scale: 1.35, duration: 520, onComplete: () => fx.destroy() }); }
